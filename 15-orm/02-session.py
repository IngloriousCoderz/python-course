...
with engine.begin() as conn:
    ...
    conn.execute(text("INSERT INTO points (x, y) VALUES (:x, :y)"), [
                 {'x': 1, 'y': 1}, {'x': 2, 'y': 4}, {'x': 6, 'y': 8}, {'x': 9, 'y': 10}, {'x': 11, 'y': 12}, {'x': 13, 'y': 14}])

# session uses connection internally
...
# session is also able to commit
with Session(engine) as session:
    result = session.execute(
        text("UPDATE points SET y=:y WHERE x=:x"),
        [{"x": 9, "y": 11}, {"x": 13, "y": 15}],
    )
    session.commit()
